#!/usr/bin/env python3
"""
OCR Test Script - Fixed Version
Properly extracts text from Tesseract TSV output
"""
...
def run_ocr(image_path, output_base):
    ...
    """
    # Run Tesseract with TSV output (includes confidence scores)
    cmd = [
    ...
    # Parse TSV for confidence and text
    confidence = 0
    text = ""
                ...
                for line in lines[1:]:  # Skip header
                    ...
                    if len(parts) > 10:
                        try:
                            ...
                            if conf_val > 0 and word:  # Filter out zero confidence and empty words
                                words.append(word)
...
def test_simple_text():
    ...
    if test_text and text:
        accuracy = len(set(text.lower().replace(' ', '')) & set(test_text.lower().replace(' ', ''))) / len(set(test_text.lower().replace(' ', ''))) * 100
...
def test_numbers():
    ...
    accuracy = 0
    if test_text and text:
        accuracy = len(set(text.replace(' ', '')) & set(test_text.replace(' ', ''))) / len(set(test_text.replace(' ', ''))) * 100
        print(f"Character accuracy: {accuracy:.2f}%")
...
def test_with_preprocessing():
    ...
    if test_text:
        if text_raw:
            accuracy_raw = len(set(text_raw.lower().replace(' ', '')) & set(test_text.lower().replace(' ', ''))) / len(set(test_text.lower().replace(' ', ''))) * 100
        if text_pp:
            accuracy_pp = len(set(text_pp.lower().replace(' ', '')) & set(test_text.lower().replace(' ', ''))) / len(set(test_text.lower().replace(' ', ''))) * 100
...
def main():
    ...
    # Save results
    results_path = OCR_OUTPUT / "test_results_fixed.json"
